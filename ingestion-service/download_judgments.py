...
# ──────────────────────────────────────────────
# 📥 Download & Ingest AKN HTML Judgment
# ──────────────────────────────────────────────
def download_and_ingest_akn(url):
    try:
        ...
        resp = requests.post(INGEST_API, json={"filename": filename, "content": encoded}, timeout=60)
        print(f"📤 Ingested: {filename} → {resp.status_code}")
    except Exception as e:
        print(f"❌ Error ingesting AKN {url}: {e}")
...
if __name__ == "__main__":
    ...
    for page in range(1, total_pages + 1):
        ...
        for summary in summary_links:
            ...
            if akn_url:
                sleep(0.5)
                download_and_ingest_akn(akn_url)
            else:
                print(f"⚠️ No AKN link found in summary: {summary}")

    print("\n✅ All AKN judgments downloaded and ingested.")
