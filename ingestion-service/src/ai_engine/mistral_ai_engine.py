...
class MistralAIEngine(AIEngine):
    def __init__(self):
        ...
        # Mistral pipeline (for summarization + QA via prompts)
    ...
    def summarize(self, text: str) -> str:
        ...
        try:
            output = self.llm(prompt, max_new_tokens=128, do_sample=False)[0]["generated_text"]
    ...
    def answer_question(self, question: str, context: str) -> Dict[str, str]:
        ...
        prompt = f"Context:\n{context}\n\nQuestion: {question}\nAnswer:"
        try:
            result = self.llm(prompt, max_new_tokens=128, do_sample=False)[0]["generated_text"]
